...
class ForumPostController extends \Atrakeur\Forum\Controllers\AbstractPostForumController {
	...
	public function postNewMessage($categoryId, $categoryUrl, $topicId, $topicUrl)
	{
		...
		if ($validator->passes())
		{
			...
			$topic_object = \Atrakeur\Forum\Models\ForumTopic::where("id", "=", $topicId)->first();
			if(isset($topic_object->id) && $topic_object->id > 0){
				$topic_object->last_reply = date("Y-m-d H:i:s");
				$topic_object->save();
			}
